...
module.exports = class AuthorizationCode {
  ...
  async getToken(params, httpOptions) {
    ...
    console.log('GET TOKEN PARAMS', JSON.stringify(params,null,2), JSON.stringify(httpOptions, null,2))
    const parameters = GrantTypeParams.forGrantType('authorization_code', this.#config.options, params);
    console.log('GET TOKEN PARAMETERS RESPONSE', parameters)
    return await this.#client.request(this.#config.auth.tokenPath, parameters.toObject(), httpOptions)
      .then(res => {
        console.log('REQUEST RESPONSE', JSON.stringify(res, null, 2))
